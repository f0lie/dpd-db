...
from db.models import DpdHeadword, DpdRoot, FamilyRoot, Lookup, SBS, Russian
...
def home():
    ...
    if query:
        if query.isalpha():
            result = (
                db.session.query(Lookup).filter(Lookup.lookup_key == (query)).first()
            )

            if not result:
                query = query.lower()
                result = (
                    db.session.query(Lookup)
            ...
            if result:
                ...
                if result.roots:
                    ...
                    root_results = (
                        db.session.query(DpdRoot)
                        .filter(DpdRoot.root.in_(roots_list))
                        .all()
                    )
                    for r in root_results:
                        frs = db.session.query(FamilyRoot).filter(
            ...
            query = int(query)
            headword = (
                db.session.query(DpdHeadword).filter(DpdHeadword.id == query).first()
            )
            if headword:
                print(headword)
