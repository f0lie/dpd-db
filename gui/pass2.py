...
class Pass2Data:
    def __init__(
        ...
        self.exceptions: List[int] = [
            ...
            19159,
            19162,
        ]  # irrelevant words which appear freqently
    ...
    def update_phrase_exceptions(self, phrase_exception: str) -> None:
        ...
        self.tried_dict["phrase_exceptions"] = pali_list_sorter(
            self.phrase_exceptions_set
        )
        self.save_tried_dict()
...
class WordData:
    def __init__(self, word) -> None:
        self.word: str = word
        self.subword: str
        self.search_pattern: str = rf"(^|\[|\{{|\s)({self.word})(\s|[.,;:!?]|\]|\}}$)"
    ...
    def update_sutta_example(self, sutta_example):
        ...
        parts = re.split(rf"\b{self.word}\b", self.example)
...
def start_from_where_gui(p2d: Pass2Data):
    ...
    start_window: sg.Window = sg.Window("Pass2", layout, location=(400, 400))
        ...
        if event == sg.WINDOW_CLOSED:
            ...
            # start from a specific word
                ...
                try:
                    ...
                    p2d.main_window["messages"].update(
                        value=f"starting from {user_input}", text_color="white"
                    )
                    break
                except ValueError:
...
def window_options(p2d: Pass2Data):
    ...
    pass2_layout = [
        [sg.Text("index", size=(15, 1)), sg.Text("", key="pass2_index", size=(50, 1))],
        [sg.Text("word", size=(15, 1)), sg.Text("", key="pass2_word", size=(50, 1))],
        [
            sg.Text("", size=(15, 1)),
        ],
        [
        ...
        [
            ...
            sg.Multiline("", key="pass2_example", size=(50, 10), no_scrollbar=True),
        ],
        [
            sg.Text("", size=(15, 1)),
        ],
        ...
        [
            ...
            sg.Input("", key="pass2_phrase_exception", size=(46, 1)),
    ...
    pass2_window = sg.Window(
        "Pass2", pass2_layout, size=(width, height), finalize=True, resizable=True
    )
...
def pass2_gui(p2d: Pass2Data) -> tuple[Pass2Data, WordData]:
    ...
    for index, word in enumerate(p2d.word_list):
        ...
        # update the gui
        p2d.pass2_window["pass2_index"].update(
            value=f"{index} / {p2d.word_list_length}"
        )
...
def get_headwords(p2d: Pass2Data, word_to_find: str) -> List[int] | None:
    """Get the headwords of an inflected word."""
    results: Optional[Lookup]
    results = p2d.db_session.query(Lookup).filter_by(lookup_key=word_to_find).first()
...
def get_deconstruction_headwords(p2d: Pass2Data, word_to_find: str) -> list[int] | None:
    ...
    if results and results.deconstructor:
        ...
        for inflection in inflections:
            results = (
                p2d.db_session.query(Lookup).filter_by(lookup_key=inflection).first()
            )
            if results:
...
def check_example_headword(p2d: Pass2Data, wd: WordData, id_list: List[int]) -> None:
    if id_list:
        for id in id_list:
            if p2d.continue_flag == "new":
                return
            ...
            wd.update_id(id)
            dpd_headword = p2d.db_session.query(DpdHeadword).filter_by(id=wd.id).first()

            if dpd_headword:
                wd.update_headword(dpd_headword.lemma_1)

                # test not in tried_dict
                test1 = wd.sutta_example not in p2d.tried_dict.get(p2d.book, {}).get(
                    wd.word, {}
                ).get(wd.id, [])
                ...
                test3 = (
                    any(item in dpd_headword.source_1 for item in p2d.commentary_list)
...
def has_no_meaning_or_example(headword: DpdHeadword) -> bool:
    """ "Test the pali_word"""
...
def update_gui(p2d: Pass2Data, wd: WordData, headword: DpdHeadword):
    """
    ...
    p2d.pass2_window["pass2_example"].print(
        wd.example_start, text_color="darkgray", end=""
    )  # type:ignore
    p2d.pass2_window["pass2_example"].print(wd.word, text_color="dodgerblue", end="")  # type:ignore
    p2d.pass2_window["pass2_example"].print(
        wd.example_end, text_color="darkgray", end=""
    )  # type:ignore
...
def choose_route(p2d: Pass2Data, wd: WordData, headword: DpdHeadword):
    """
...
def test_words_in_construction(
    p2d: Pass2Data, wd: WordData, dpd_headword: DpdHeadword
) -> None:
    if (
        dpd_headword
        ...
        construction_clean = re.sub(r" >.+?\+ ", " + ", dpd_headword.construction)
...
def update_main_window(p2d: Pass2Data, wd: WordData):
    ...
    if p2d.continue_flag == "new":
        ...
        headword = (
            p2d.db_session.query(DpdHeadword).filter(DpdHeadword.id == wd.id).first()
        )
        if headword:
...
def is_phrase_exception(p2d, wd):
    for phrase_exception in p2d.phrase_exceptions_set:
        if wd.word in phrase_exception and phrase_exception in wd.example:
            return True
    return False
