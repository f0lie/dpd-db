...
def display_dps_summary(values, window, sg, original_values):
    ...
    for value in values:
        if value in dps_values_list:
            if values[value] and value not in excluded_fields:
                ...
                color = (
                    "yellow"
                    if str(original_values.get(value.replace("dps_", "")))
                    != str(values[value])
                    else "white"
                )
    ...
    window = sg.Window("Summary", summary_layout, location=(250, 0), size=(900, 1000))
    ...
    while True:
        ...
        if (
            ...
            or event == "dps_ok_button"
            or event == "dps_edit_button"
        ):
            break
    window.close()
    return event
...
def stash_values_from(dpspth, values, num, window, error_field):
    ...
    values_to_stash = {
        key_template.format(""): values[key]
        for key, key_template in zip(keys_to_stash, KEYS_TEMPLATE)
        if key in values
    }
...
def unstash_values_to(dpspth, window, num, error_field):
    ...
    for key_template in KEYS_TEMPLATE:
        ...
        if key_to_fetch in unstashed_values:
            ...
            print(
                f"Updated window[{key_to_update}] with value: {unstashed_values[key_to_fetch]}"
            )
...
def dps_example_save(
    pth: ProjectPaths, values: dict[str, str], window, example_attr: str
) -> None:
...
def translate_with_openai(
    ...
    # Choosing sentence
    sentence = (
        main_sentence if number == "0" else [ex_1, ex_2, ex_3, ex_4][int(number) - 1]
    )
...
def copy_and_split_content(
    sugestion_key, meaning_key, lit_meaning_key, error_field, window, values
):
    """
...
def ru_check_spelling(dpspth, field, error_field, values, window):
    ...
    if correction:
        ...
        window[error_field].set_size(
            (50, num_lines)
        )  # Adjust the size of the Text element based on the number of lines
...
def ru_edit_spelling(dpspth):
    subprocess.Popen(["code", dpspth.ru_user_dict_path])


def tail_log():
    subprocess.Popen(
        ["gnome-terminal", "--", "tail", "-n", "+0", "-f", "/home/deva/logs/gui.log"]
    )
...
def save_gui_state_dps(dpspth, values, words_to_add_list):
    ...
    print(
        f"[green]saving gui state (2), values:{len(values)}, words_to_add_list: {len(words_to_add_list)}"
    )
...
def load_gui_state_dps(dpspth):
    ...
    print(
        f"[green]loading gui state (2), values:{len(values)}, words_to_add_list: {len(words_to_add_list)}"
    )
...
def send_sutta_study_request(word, sutta, source):
    try:
        ...
        params = {
            "sutta_panels": [{"sutta_uid": sutta_uid, "find_text": word}],
            "lookup_panel": {"query_text": word},
        }
        ...
        if row.get("id") == current_id:
            ...
            for i in range(total_examples):
                ...
                if sbs_example and all(
                    ...
                    window["dps_sbs_source_4"].update(
                        row.get(f"sbs_source_{example_index + 1}", "")
                    )
                    window["dps_sbs_sutta_4"].update(
                        row.get(f"sbs_sutta_{example_index + 1}", "")
                    )
                    window["dps_sbs_example_4"].update(sbs_example)
                    window["dps_sbs_chant_pali_4"].update(
                        row.get(f"sbs_chant_pali_{example_index + 1}", "")
                    )
                    window["dps_sbs_chant_eng_4"].update(
                        row.get(f"sbs_chant_eng_{example_index + 1}", "")
                    )
                    window["dps_sbs_chapter_4"].update(
                        row.get(f"sbs_chapter_{example_index + 1}", "")
                    )
                    ...
                    return (
                        archived_example_index  # Return updated index for the next call
                    )
