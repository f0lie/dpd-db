...
def filter_and_update(
    ...
    words_to_update = (
        ...
        .filter(column_to_filter == filter_value)
        .all()
    )
    ...
    for __word__, related in words_to_update:
        ...
        if old_value or update_value:
            ...
            console.print(
                f"[bold bright_yellow]{__word__.id} {related_column_to_update.key}:"
            )
            print()
            print(f"{old_value}")
            print()
...
def update_column_for_some_criteria(source_value):
    ...
    for __word__ in rows_to_update_sbs:
        ...
        if not old_value:
            ...
            console.print(
                f"[bold bright_yellow]{__word__.id} {__word__.lemma_1} {__word__.sbs.sbs_patimokkha}"
            )
    ...
    for __word__ in rows_to_update:
        if not __word__.sbs:
            ...
            console.print(
                f"[bold bright_yellow]Added {__word__.id} {__word__.lemma_1} {__word__.sbs.sbs_patimokkha}"
            )
            count_added += 1
        else:
            old_value = __word__.sbs.sbs_patimokkha
    ...
    console.print(
        f"[bold bright_green]Total count of already has: {count_sbs_patimokkha}"
    )
...
update_column_for_some_criteria("VIN1.2.11")
