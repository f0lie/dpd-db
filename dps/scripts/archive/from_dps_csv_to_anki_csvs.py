...
def classes(df, sbs_ped_link):
    ...
    for sbs_class_value in unique_sbs_class_values:
        ...
        full_file_path = os.path.join(
            dpspth.anki_csvs_dps_dir, "pali_class", "classes", file_name
        )
    ...
    for sbs_class_value in unique_sbs_class_values:
        ...
        if start_value <= sbs_class_value_int <= end_value:
            ...
            filtered_ru_df = df_original[
                df_original["sbs_class_anki"] == sbs_class_value
            ]
    ...
    output_path_ru = os.path.join(
        dpspth.anki_csvs_dps_dir, "pali_class", "class_ru.csv"
    )
...
def suttas_class(df, sbs_ped_link):
    ...
    for category in unique_categories:
        if not pd.isnull(category) and category != "":
            ...
            output_path = os.path.join(
                dpspth.anki_csvs_dps_dir, "pali_class", "suttas", output_filename
            )
            filtered_df.to_csv(output_path, sep="\t", index=False, header=True)
