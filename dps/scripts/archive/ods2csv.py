...
class ReadOds:
    def __init__(self, filename, sheets_name):
        ...
        for sheet_name in sheets_name:
            ...
            if sheet_rows is not None:
                ...
                n_row = len(header)
                data = [
                    dict(zip(header, (self.get_columns_in_row(row)[:n_row])))
    ...
    def save_csv(self):
        ...
        for sheet_name in self.df:
            self.df[sheet_name].sort_values(
                by=["lemma_1"],
                ignore_index=True,
                inplace=True,
            ...
            self.df[sheet_name].to_csv(
                dpspth.dps_csv_path, sep="\t", index=False, quoting=1
            )
    ...
    def get_bold_styles(self):
        ...
        all_bolds = [
            item
            for item in all_text_properties
            if item.has_attr("fo:font-weight") and item["fo:font-weight"] == "bold"
        ]
    ...
    def get_rows_in_sheet(self, sheet_name):
        console.print(
            f"[bold green]processing cell data for sheet {sheet_name.lower()}"
        )
        ...
        if isinstance(current_sheet, BeautifulSoup):
            ...
            console.print(
                f"[bold red]current_sheet is not a BeautifulSoup object for sheet {sheet_name}"
            )
            return None
