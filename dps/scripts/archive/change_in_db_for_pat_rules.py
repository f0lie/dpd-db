...
def filter_and_update():
    ...
    words_to_update = (
        ...
        .filter(DpdHeadword.lemma_1.endswith("sikkhāpada"), DpdHeadword.meaning_2 == "")
        .all()
    )
...
def filter_and_update_from_file(pth: ProjectPaths):
    ...
    print(
        "First few words:", [word.lemma_2 for word in words_to_update[:5]]
    )  # Debug info
    ...
    for word in words_to_update:
        ...
        if rule_number:
            ...
            print(
                f"Failed: {word.lemma_2}"
            )  # This will let us know which words did not get updated
...
def update_family_set_based_on_conditions(pth: ProjectPaths):
    ...
    for word in db_session.query(DpdHeadword).filter_by(
        family_set="bhikkhupātimokkha rules", meaning_2=""
    ):
        word.family_set = ""

        print(
            f"id: {word.id}, meaning_2 : {word.meaning_2} family_set(new): {word.family_set}"
        )
