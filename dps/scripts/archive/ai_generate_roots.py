...
def roots_meaning_generating(lang):
    ...
    subquery = (
        db_session.query(
            DpdHeadword.root_key, func.count(DpdHeadword.id).label("headword_count")
        )
    ...
    for counter, root in enumerate(roots):
        if lang == "pali":
            ru_root_meaning = translate_pali_roots(
                root.root_clean, root.root_meaning, root.root_example
            )
            ...
            if ru_root_meaning:
                ...
                print(
                    f"{counter + 1}. {root.root_clean}, {root.root_count} {ru_root_meaning}"
                )

        if lang == "sk":
            ru_root_meaning = translate_sk_roots(
                root.root_clean, root.sanskrit_root_meaning
            )
            ...
            if ru_root_meaning:
                ...
                if lang == "pali":
                    print(
                        f"{counter + 1}. {root.root_clean}, {root.root_meaning} {ru_root_meaning}"
                    )
                if lang == "sk":
                    print(
                        f"{counter + 1}. {root.sanskrit_root}, {root.sanskrit_root_meaning} {ru_root_meaning}"
                    )
