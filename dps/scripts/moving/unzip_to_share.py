...
if os.path.exists(dpd_goldendict_src):
    ...
    with ZipFile(dpd_goldendict_src, "r") as zipObj:
        ...
        members_to_extract = [
            m
            for m in members
            if m.startswith("dpd-deconstructor/") or m.startswith("dpd-grammar/")
        ]
    ...
    print(
        "\033[1;32m gdict deconstructor and grammar has been unpacked to share folder \033[0m"
    )
...
if os.path.exists(dpd_mdict_src):
    ...
    # Print completion message in green color
    print(
        "\033[1;32m mdict deconstructor and grammar has been unpacked to share \033[0m"
    )
