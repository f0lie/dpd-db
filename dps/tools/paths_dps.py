...
class DPSPaths:
    def __init__(self, base_dir: Optional[Path] = None, create_dirs=True):
        ...
        self.pali_class_grammar_dir = base_dir.joinpath(
            Path("dps/csvs/anki_csvs/pali_class/grammar/")
        )
        ...
        self.dpd_dps_full_freq_path = base_dir.joinpath(
            Path("dps/csvs/dpd_dps_full_freq.csv")
        )
        self.ai_ru_suggestion_history_path = base_dir.joinpath(
            Path("dps/csvs/ai_ru_suggestion_history.csv")
        )
        self.ai_ru_notes_suggestion_history_path = base_dir.joinpath(
            Path("dps/csvs/ai_ru_notes_suggestion_history.csv")
        )
        self.ai_en_suggestion_history_path = base_dir.joinpath(
            Path("dps/csvs/ai_en_suggestion_history.csv")
        )
        ...
        self.translation_example_path = base_dir.joinpath(
            Path("dps/csvs/tranlslation_examples.csv")
        )
        self.sbs_archive = base_dir.joinpath(Path("dps/sbs_csvs/sbs_archive.tsv"))
        self.sbs_example_corrections = base_dir.joinpath(
            Path("dps/csvs/sbs_example_corrections.tsv")
        )
        self.ru_apply_path = base_dir.joinpath(Path("temp/ru_apply.csv"))

        self.ai_for_batch_api_dir = base_dir.joinpath(
            Path("dps/csvs/ai_for_batch_api/")
        )
        self.ai_from_batch_api_dir = base_dir.joinpath(
            Path("dps/csvs/ai_from_batch_api/")
        )
        ...
        # /tests
        self.dps_internal_tests_path = base_dir.joinpath(
            Path("dps/csvs/dps_internal_tests.tsv")
        )
        self.dps_internal_tests_replaced_path = base_dir.joinpath(
            Path("dps/csvs/dps_internal_tests_replaced.tsv")
        )
        ...
        self.ru_user_dict_path = base_dir.joinpath(
            Path("dps/rus/russian_words_user_dict.txt")
        )
        self.text_to_add_path = base_dir.joinpath(Path("temp/text.txt"))
        self.dpd_dps_concise_txt_path = base_dir.joinpath(
            Path("temp/dpd_dps_concise.txt")
        )
        ...
        self.anki_media_dir = base_dir.joinpath(
            Path(
                "/home/deva/.var/app/net.ankiweb.Anki/data/Anki2/deva/collection.media/"
            )
        )
