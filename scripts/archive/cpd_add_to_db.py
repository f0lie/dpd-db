...
def test_1(cpd, headwords):
    """Test if cpd.lemma_1 in db headwords."""
    test_1 = []
    for i in cpd:
        if i.stem:
            if i.lemma_1 in headwords:
                test_1 += [i.lemma_1]
    print(f"[green]{'test 01: lemma_1 already in db':<40}", end="")
    if test_1:
        print()
...
def test_7(cpd):
    """Test if stem + pattern = lemma_1 clean."""
    test_7 = []
    for i in cpd:
        if i.stem and i.stem != "-":
            lemma_clean = re.sub(r" \d.*$", "", i.lemma_1)
            pattern_clean = re.sub(" .*$", "", i.pattern)
            stem_pattern = f"{i.stem}{pattern_clean}"
            if lemma_clean != stem_pattern:
                test_7 += [i.lemma_1]
    print(f"[green]{'test 07: stem pattern equals lemma_1':<40}", end="")
    if test_7:
        print()
...
def test_12(cpd, headwords_clean):
    ...
    for i in cpd:
        lemma_clean = re.sub(r" /d.*$", "", i.lemma_1)
        if lemma_clean not in headwords_clean:
            test_12 += [i.lemma_1]
