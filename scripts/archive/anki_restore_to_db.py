...
def main():
    ...
    for d in data_dict:
        ...
        if note_id and int(note_id) > 77297:
...
def check_unique(db_session, i):
    ...
    existing_word = (
        ...
        .filter((DpdHeadword.id == i.id) | (DpdHeadword.lemma_1 == i.lemma_1))
        .first()
    )
...
def update_fields(id: str, i: DpdHeadword, data_dict: dict):
    ...
    for attr in dir(i):
        if not attr.startswith("_"):
            ...
            anki_value = (
                data_dict[id]["note"][attr] if attr in data_dict[id]["note"] else None
            )
