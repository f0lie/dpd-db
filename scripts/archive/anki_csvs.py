...
def commentary(pth: ProjectPaths, dpd_db):
    ...
    for i in dpd_db:
        if i.meaning_1 and not i.example_1:
...
def pass1(pth: ProjectPaths, dpd_db):
    ...
    rows = []
    for i in dpd_db:
        if not i.meaning_1 and "pass1" in i.origin:
            rows.append(pali_row(i))
...
def roots(pth: ProjectPaths, db_session: Session, roots_db):
    ...
    for root in roots_list:
        count = (
            db_session.query(DpdHeadword).filter(DpdHeadword.root_key == root).count()
        )
    ...
    dpd_df.sort_values(
        by=["Root"], inplace=True, ignore_index=True, key=lambda x: x.map(pali_sort_key)
    )
    dpd_df.to_csv(
        pth.roots_csv_path,
        sep="\t",
