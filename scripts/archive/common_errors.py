...
def family_compound_contains_plus():
    ...
    filtered_db = (
        db_session.query(DpdHeadword)
        .filter(DpdHeadword.family_compound.contains("+"))
        .all()
    )
...
def family_root_missing():
    ...
    filtered_db = (
        db_session.query(DpdHeadword)
        .filter(DpdHeadword.family_root == "", DpdHeadword.root_key)
        .all()
    )
...
def problem_patterns():
    ...
    with open("../inflection generator/output/InflectionTemplatess dict", "rb") as p:
        inflection_templates_dict = pickle.load(p)
    patterns = inflection_templates_dict.keys()

    filtered_db = (
        db_session.query(DpdHeadword)
        .filter(DpdHeadword.pattern, DpdHeadword.pattern.notin_(patterns))
        .all()
    )
...
def wrong_pos():
    ...
    filtered_db = (
        db_session.query(DpdHeadword).filter(DpdHeadword.pos.notin_(pos_list)).all()
    )
    if len(filtered_db) > 0:
        for i in filtered_db:
