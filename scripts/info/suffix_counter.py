...
def main_pandas():
    ...
    for i in dpd_db:
        if i.meaning_1 and i.derivative in ["kita", "kicca", "taddhita"]:
            data.append(
                [i.derivative, i.suffix, i.pos, i.lemma_1, i.meaning_1, i.construction]
            )

    df = pd.DataFrame(
        data,
        columns=["derivative", "suffix", "pos", "lemma_1", "meaning", "construction"],
    )

    # add counts of suffixes and pos
    df["count_suffix"] = df.groupby(["derivative", "suffix"])["derivative"].transform(
        "count"
    )
    df["count_pos"] = df.groupby(["derivative", "suffix", "pos"])[
        "derivative"
    ].transform("count")

    df.sort_values(
        by=["count_suffix", "suffix", "derivative", "count_pos", "pos", "lemma_1"],
        inplace=True,
        ignore_index=True,
