...
writer = MDictWriter(
    d, "Big5 dictionary", 'This is a test for the "Big5" encoding.', encoding="big5"
)
writer.write(outfile)
outfile.close()
...
writer = MDictWriter(
    d, "GBK dictionary", 'This is a test for the "GBK" encoding', encoding="gbk"
)
writer.write(outfile)
outfile.close()
...
keyfile = io.open(
    "example_output/encrypted_external_regcode.key", "w", encoding="ascii"
)
keyfile.write(key)
keyfile.close()
...
keyfile = io.open(
    "example_output/encrypted_external_regcode_device_id.key", "w", encoding="ascii"
)
keyfile.write(key)
keyfile.close()
...
try:
    writer = MDictWriter(
        d,
        "LZO compressed dictionary",
        "This tests the LZO compression type.",
        compression_type=1,
    )
    writer.write(outfile)
...
writer = MDictWriter(
    d3, "Dictionary with MDD file", "This dictionary tests MDD file handling."
)
...
writer = MDictWriter(
    d_mdd,
    "Dictionary with MDD file",
    "This dictionary tests MDD file handling.",
    is_mdd=True,
)
