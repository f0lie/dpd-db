    ...
    class Testing_pysalsa20(pySalsa20.Salsa20):
        def salsa20core(self, input, nRounds):
            ...
            try:
                ...
                msg = (
                    "*** Your libsalsa20 does not support the "
...
def t32(a):
    return tuple(trunc32(x) for x in a)
...
def test_add32(add32, name):
    ...
    print("Seconds per call:", duration / nCalls, "--", nCalls / duration, "calls/sec")
    return passed
    ...
    for j in range(-32, 33):
        for i in range(32):
            ...
            if x != y:
                print(
                    name
                    + "( 0x%08x, %d ) => 0x%08x, should be 0x%08x."
...
def test(module, module_name):
    ...
    if 1 and passed:  # one known good 8-round test vector
        ...
        s20 = salsa20_test_classes[module_name](key, IV, rounds)
        plaintxt = s20.encryptBytes(ciphertxt)
        if message == plaintxt and ciphertxt[:64] == out64:
            print("    *** vector 1 good ***")
        else:
            print("    *** vector 1 bad ***")
            passed = False
    ...
    if 1 and passed:  # one known good 8-round test vector
        ...
        if (
            message == plaintxt
            and ciphertxt[:64] == out64
            and ciphertxt[65472:] == out65536
        ):
            print("    *** vector 2 good ***")
        else:
            print("    *** vector 2 bad ***")
    ...
    for name in module_names:
        if name == "pureSalsa20":
            try:
                ...
                if verbose:
                    print("Problem importing pureSalsa20")
        elif name == "pySalsa20":
            try:
                global pySalsa20
                import pySalsa20
