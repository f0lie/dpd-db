...
def get_bjt_file_names(books: list[str] | str) -> list[str]:
    ...
    for page in pages:
        ...
        for entry in entries:
            ...
            if footnotes_inline and footnotes:
                text = replace_footnotes(text, footnotes, file_name, page_number)
...
def replace_footnotes(text, footnotes, file_name, page_number):
    footnote_links = re.findall(r"""\{(\d*)\}""", text)
    for fl in footnote_links:
        num = int(fl)
        try:
            footnote_num = footnotes[num - 1]["text"]
            text = re.sub(
                rf"""\{{{num}\}}""", rf" [{footnote_num}]", text, flags=re.DOTALL
            )
        except IndexError:
