...
def extract_pos():
    ...
    df["pos"] = df["cpd"].str.extract(
        "(grd\\.|m\\. and n\\.|ind\\.|pp\\.|mfn|f\\.|f<\\/i>|m\\.|n\\.|aor\\.|m<\\/i>\\(<i>fn|m<\\/i>\\(<i>n\\?</i>|root|v\\.l\\.|w\\.r\\.|reading|ifc|v\\. l\\.|pr\\.|√|n<\\/b>|fut\\.|cond\\.|opt\\.|w\\. r\\. |<i>f<\\/i>|see<\\/i>|<i>see |v\\. r\\.|adv\\.|<i>read<\\/i><i>read |m<\\/i>.|abs\\.|pot\\.|<i>num\\.|indecl\\.)"
    )
...
def extract_verb():
    ...
    df["verb"] = (
        df["cpd"]
        .str.findall("<i>(caus\\.|denom\\.|pass\\.|pass\\. caus\\.)")
        .str.join(", ")
    )
...
def extract_case():
    ...
    df["case"] = (
        df["cpd"]
        .str.findall("\swith (nom|acc|instr|dat|abl|gen|loc|voc)")
        .str.join(",")
    )
...
def extract_root():
    ...
    df["root"] = df["cpd"].str.extract("(√.+?)(,| |\\(|\\)|\\]|\d|;|\\/|\\<)")[0]
