...
def setup(pth: ProjectPaths):
    ...
    (all_inflections_nofirst, all_inflections_nolast) = make_all_inflections_nfl_nll(
        all_inflections_set
    )
...
def import_sandhi_rules(pth: ProjectPaths):
    ...
    sandhi_rules_df = pd.read_csv(pth.sandhi_rules_path, sep="\t", dtype=str)
    ...
    dupes = sandhi_rules_df[
        sandhi_rules_df.duplicated(["chA", "chB", "ch1", "ch2"], keep=False)
    ]
    ...
    if (
        sandhi_rules_df["chA"].str.contains(" ").any()
        or sandhi_rules_df["chB"].str.contains(" ").any()
        or sandhi_rules_df["ch1"].str.contains(" ").any()
        or sandhi_rules_df["ch2"].str.contains(" ").any()
    ):
...
def main():
    ...
    for counter, word in enumerate(unmatched_set.copy()):
        ...
        if d.count % 1000 == 0:
            print(f"{d.count:>10,} / {unmatched_len_init:<10,}{d.word}")
...
def save_timer_dict(pth: ProjectPaths, time_dict):
    ...
    df.to_csv(pth.sandhi_timer_path, mode="a", header=False, sep="\t")
...
def remove_neg(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 2:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-na", "na", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_sa(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-sa", "sa", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_su(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-su", "su", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_dur(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-dur", "dur", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_ati(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-ati", "ati", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_tta(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-tta", "tta", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_tissa(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 3:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-tissa", "tissa", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
...
def remove_double_consonants(d: DotDict) -> DotDict:
    ...
    if comp(d) not in w.matches and len(d.word) > 4:
        ...
        if d.word in all_inflections_set:
            ...
            if comp(d) not in w.matches:
                matches_dict[d.init] += [(comp(d), "xword-dc", "dc", d.path)]
                w.matches.add(comp(d))
                d.matches.add(comp(d))
                unmatched_set.discard(d.init)
    ...
    if comp(d) not in d.matches:
        ...
        if len(d.word) >= fuzzy_word_min_length:
            for i in range(len(d.word)):
                ...
                if (
                    fuzzy_word in all_inflections_nolast
                    or fuzzy_word in all_inflections_set
                ):
    ...
    if comp(d) not in w.matches:
        ...
        if len(d.word) > 0:
            for i in range(len(d.word)):
                ...
                if (
                    fuzzy_word in all_inflections_nofirst
                    or fuzzy_word in all_inflections_set
                ):
                ...
                for rule in rules:
                    ...
                    if wordA_lastletter == chA and wordB_firstletter == chB:
                        ...
                        if word2 in all_inflections_set:
                            d.path += " > back_fuzzy"
                            d.word = re.sub(f"{wordB_fuzzy}$", "", d.word, count=1)
                            d.word = re.sub(f"{wordA_fuzzy}$", word1, d.word, count=1)
                            # d.back = re.sub(
    ...
    if comp(d) not in w.matches:
        for x in range(0, len(d.word) - 1):
            ...
            for y in range(0, len(d.word[-1 - x :]) - 1):
                ...
                if (
                    wordA in all_inflections_set
                    and wordB in all_inflections_set
                    and wordC in all_inflections_set
                ):
                    d.front = f"{d.front}{wordA} + "
                    d.word = wordB
                    ...
                    if comp(d) not in w.matches:
                        matches_dict[d.init] += [(comp(d), d.comm, "0,0", d.path)]
                        w.matches.add(comp(d))
                        d.matches.add(comp(d))
                        unmatched_set.discard(d.init)
                ...
                if wordA in all_inflections_set:
                    for rule in rules:
                        ...
                        if wordB_lastletter == chA and wordC_firstletter == chB:
                            ...
                            if (
                                wordA in all_inflections_set
                                and word2 in all_inflections_set
                                and word3 in all_inflections_set
                            ):
                                d.front = f"{d.front}{wordA} + "
                                d.word = word2
                ...
                for rulex in rules:
                    ...
                    if wordA_lastletter == chAx and wordB_firstletter == chBx:
                        word1 = wordA[:-1] + ch1x
                        word2 = ch2x + wordB[1:]
                        ...
                        for ruley in rules:
                            ...
                            if wordB_lastletter == chAy and wordC_firstletter == chBy:
                    ...
                    for rulex in rules:
                        ...
                        if wordA_lastletter == chAx and wordB_firstletter == chBx:
                            ...
                            for ruley in rules:
                                ...
                                if (
                                    wordB_lastletter == chAy
                                    and wordC_firstletter == chBy
                                ):
                                    word2 = (ch2x + wordB[1:])[:-1] + ch1y
                                    word3 = ch2y + wordC[1:]
                                    ...
                                    for rulez in rules:
                                        ...
                                        if (
                                            wordC_lastletter == chAz
                                            and wordD_firstletter == chBz
                                        ):
                                            ...
                                            if (
                                                ...
                                                and word2 in all_inflections_set
                                                and word3 in all_inflections_set
                                                and word4 in all_inflections_set
                                            ):
                                                d.front = (
                                                    f"{d.front}{word1} + {word2} + "
                                                )
                                                d.word = word3
