...
def process_long_words(g: GlobalVars):
    ...
    for counter, (clean_word, values) in enumerate(g.variations_dict.items()):
        ...
        if clean_word not in g.hyphenations_dict:
            if only_do_hyphenated_words and not has_hyphenated_word(dirty_words):
                continue
            ...
            print(
                "[green]which version would you like? number [white]m[/white]anual [white]o[/white]k [white]p[/white]ass e[white]x[/white]it ",
                end="",
            )
            choice = input()
            print()
            ...
            elif choice == "o" or choice.isdigit() and dcount == 0:
        ...
        for i in db_list:
            for field, field_name in [
                ...
                if replace_me in field:
                    ...
                    field_old_highlight = field.replace(
                        replace_me, f"[cyan]{replace_me}[/cyan]"
                    )
                    print(f"[green]{field_old_highlight}")
                    print()

                    field_new_highlight = field_new.replace(
                        g.spelling_chosen, f"[cyan]{g.spelling_chosen}[/cyan]"
                    )
