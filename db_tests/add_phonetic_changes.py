    ...
    for c_counter, c in enumerate(csv):
        ...
        for i_counter, i in enumerate(db):
            if i.meaning_1 and i.construction and i.lemma_1 not in c.exceptions:
                ...
                # test construction
                if (
                    (c.initial in construction_clean or c.initial in base_clean)
                    and c.final in i.lemma_clean
                    and c.correct not in i.phonetic
                    and not (c.without in construction_clean or c.without in base_clean)
                ):
