...
def add_update_phonetic(db_session, db, csv):
    for c_counter, c in enumerate(csv):
        ...
        for i_counter, i in enumerate(db):
            ...
            if i.meaning_1 and i.construction and i.lemma_1 not in c.exceptions:
                construction_clean, base_clean = clean_construction_base(i)

                # test construction
                if c.initial in construction_clean or c.initial in base_clean:
                    c_compiled, b_compiled = compile_previous_next(
                        c, construction_clean, base_clean
                    )

                    if (
                        (c_compiled in i.lemma_1 or b_compiled in i.lemma_1)
                        ...
                        if c.wrong and c.wrong in i.phonetic:
                            i.phonetic = re.sub(
                                rf"\b{c.wrong}\b", str(c.correct), str(i.phonetic)
                            )
        ...
        print(
            f"[green]{'auto_updated':<15}: [white]{db_search_string(auto_updated)} [{len(auto_updated)}]"
        )
        print(
            f"[green]{'auto_added':<15}: [white]{db_search_string(auto_added)} [{len(auto_added)}]"
        )

        manual_update_string = db_search_string(manual_update)
        print(
            f"[green]{'manual_update':<15}: [white]{manual_update_string} [{len(manual_update)}]"
        )
        pyperclip.copy(manual_update_string)

        if (auto_updated or auto_added or manual_update) and c_counter < len(csv) - 1:
